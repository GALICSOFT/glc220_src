...
INT CParticle::Create(PLC_DEVICE pDev)
{
	INT hr;

	m_pDev = pDev;

	hr = LcDev_CreateTextureFromFile(NULL, &m_pTex, (void*)MEDIA_DIR"tex/star128.tga");
	if(LC_FAILED(hr))
		return LC_EFAIL;
...
void CParticle::Render()
{
	...
	m_pDev->GetTransform(LC_TS_VIEW, &mtV);
	...
	m_pDev->SetRenderState(LC_RS_BLEND, LC_ST_ENABLE);
	m_pDev->SetRenderState(LC_RS_BLEND_SRC, LC_ST_SRC_ALPHA);
	m_pDev->SetRenderState(LC_RS_BLEND_DEST, LC_ST_ONE);
...
//	pVtx[3] = VtxUV1(-1.0f, 0.0f,  1.0f,  0.0f,  1.0f);
	...
	for(i=0; i<m_nPrt; ++i)
	{
		...
		m_pDev->SetTransform(LC_TS_WORLD, &mtW);
