...
INT	CMain::Init()
{
	...
	// Setup Index Buffer
	m_pIdx = new LC3D::VtxIdxW[12];


	// the front face index data
	m_pIdx[0] = LC3D::VtxIdxW(0, 1, 2);
	m_pIdx[1] = LC3D::VtxIdxW(0, 2, 3);

	// the back face index data
	m_pIdx[2] = LC3D::VtxIdxW(4, 5, 6);
	m_pIdx[3] = LC3D::VtxIdxW(4, 6, 7);

	// the top face index data
	m_pIdx[4] = LC3D::VtxIdxW( 8,  9, 10);
	m_pIdx[5] = LC3D::VtxIdxW( 8, 10, 11);

	// the bottom face index data
	m_pIdx[6] = LC3D::VtxIdxW(12, 13, 14);
	m_pIdx[7] = LC3D::VtxIdxW(12, 14, 15);

	// the left face index data
	m_pIdx[8] = LC3D::VtxIdxW(16, 17, 18);
	m_pIdx[9] = LC3D::VtxIdxW(16, 18, 19);

	// the right face index data
	m_pIdx[10] = LC3D::VtxIdxW(20, 21, 22);
	m_pIdx[11] = LC3D::VtxIdxW(20, 22, 23);
	...
	m_pObj1 = new CRndObj;
	if(m_pObj1->Create(m_pDev, (void*)MEDIA_DIR"tex/galic.png"))
	{
		delete m_pObj1;
		return LC_EFAIL;
	}
	...
	m_pObj2 = new CRndObj;
	if(m_pObj2->Create(m_pDev, (void*)MEDIA_DIR"tex/bg256x256.bmp"))
	{
		delete m_pObj2;
		return LC_EFAIL;
	}
