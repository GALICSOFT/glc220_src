...
void CGpApp::DrawShield()
{
	...
	vcTrn = m_Player.m_pos - vcScl * 32.f + LCXVECTOR2(PLAYER_WIDTH /2.F, PLAYER_HEIGHT /2.F);
...
void CGpApp::FireEnemyMissile(LCXVECTOR2 position, LCXVECTOR2 direction, FLOAT rotation)
{
	INT i;
...
void CGpApp::UpdatePlayer()
{
	...
	if (nTsbState)
	{
		if(m_pInput->IsInRect(
			(INT)m_Player.m_pos.x,
			(INT)m_Player.m_pos.y,
			(INT)(m_Player.m_pos.x + PLAYER_WIDTH),
			(INT)(m_Player.m_pos.y + PLAYER_HEIGHT)) == 1)
		{
	...
	m_Player.m_pos.x = LCXClamp(m_Player.m_pos.x, 0.F,	(FLOAT)(m_nScnW - PLAYER_WIDTH  ) );
	m_Player.m_pos.y = LCXClamp(m_Player.m_pos.y, 0.F,	(FLOAT)(m_nScnH - PLAYER_HEIGHT) );
...
void CGpApp::UpdateEnemies()
{
	...
	for(i=0; i != ENEMIESMAX; ++i)
	{
		...
		else
		{
			...
			LCXVECTOR2 direction = m_Player.m_pos - m_Enemies[i].m_pos;
			direction.Normalize();
...
void CGpApp::ScrollBackground()
{
	...
	case SCROLL_DIRECTION_LEFT:
		{
			m_BgPosition.x -= SCROLLSPEED * this->GetElapsedTimeD();
	...
	case SCROLL_DIRECTION_UP:
		{
			m_BgPosition.y -= SCROLLSPEED * this->GetElapsedTimeD();
