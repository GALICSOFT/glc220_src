...
//	if(c>20*60*20)
	...
	sprintf(str, "Default Font: 안녕하세요. ABCDEF abcdef @!#$ 12345678");
	...
	sprintf(str, "버퍼에서 생성된 폰트: Count: %d, Frame: %.f", c, fFrame);
	m_vFont->SetString(str);


	// Setting position and color
	{
		LCXVECTOR2 vcPos(5,55+ 25);

		m_vFont->SetPosition(&vcPos);
		m_vFont->SetColor(&LCXCOLOR(1, 1, 0, 1));
	}
...
int main(int argc, char* argv[])
{
	...
	char	sFile[]= MEDIA_DIR"font/seoul_hangang_l08.ttf";
	FILE* fp = NULL;
	...
	// Create font from buffer
	{
		...
		TLC_FONT	tFont(sFile, &buf, len, 24);
		if(LC_FAILED(LcDev_CreateFontFromMemory(NULL, &m_vFont, m_pSpt, &tFont)))
			goto End;
	}
