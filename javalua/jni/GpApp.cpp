...
CGpApp::CGpApp()
{
	...
	m_ScFileN	= 0		;
	memset(m_ScFile, 0, sizeof m_ScFile);
...
INT CGpApp::Create(PLC_ARG pArg)
{
	...
	// System init from script file
	if(LC_FAILED(InitScript(luaFile)))
	{
		LOGE("Init Scirpt Failed. ---------------------\n");
		return LC_EFAIL;
	}
	...
	LCXVECTOR2	vcPos = LCXVECTOR2(3, 3);
	LCXCOLOR	dCol  = LCXCOLOR(1.0F, 0.4F, 0.6F, 1);
...
INT CGpApp::Query(char* sCmd, LC_HANDLE pData)
{
	...
	else if(0 == LcStr_Stricmp((char*)"Get Script File", sCmd) )
	{
		INT		nIdx = 0;
		char*	sVal = (char*)pData;

		memcpy(&nIdx, sVal, sizeof nIdx);
		strcpy(sVal, m_ScFile[nIdx-1]);
	}

	else if(0 == LcStr_Stricmp((char*)"Get Script File Number", sCmd) )
		*((INT*)pData) = m_ScFileN;
	...
	else if(0 == LcStr_Stricmp((char*)"Get Script File with Index", sCmd) )
	{
		INT		nIdx = 0;
		char*	sVal = (char*)pData;
	...
	else if(0 == LcStr_Stricmp((char*)"Set Phase", sCmd) )
	{
		INT NewPhase =*((INT*)pData);
		hr = ChangePhase(NewPhase);
	}
	else if(0 == LcStr_Stricmp((char*)"Set Script File with Index", sCmd) )
	{
		INT		nIdx = 0;
		char*	sVal = (char*)pData;

		memcpy(&nIdx, sVal, sizeof nIdx);
		strcpy(m_ScFile[nIdx-1], sVal+ 4);
...
// GameApp Instance
INT CGpApp::ChangePhase(INT NewPhase)
{
	if(m_PhaseC == NewPhase)	// if not changed Phase
		return LC_OK;


	// lua system의 완전한 해제를 위해서 Phase를 지우고 재할 당.
	SAFE_DELETE(	m_Phase	);


	// Set Start Phase..
	CGpPhase* pNewPhase = new CGpPhase;
	LC_ARG	  arg ={&NewPhase, this};

	if(LC_FAILED(pNewPhase->Create(&arg)))
		return LC_EFAIL;
